...
import { ArtifactsLinesPaginationQuery, ArtifactsLinesPaginationQuery$variables } from './__generated__/ArtifactsLinesPaginationQuery.graphql';
import { ArtifactsLines_data$data } from './__generated__/ArtifactsLines_data.graphql';

const LOCAL_STORAGE_KEY = 'artifacts';

const artifactLineFragment = graphql`
  fragment ArtifactsLine_node on Artifact {subscribeFn
    id
    entity_type
    parent_types+
    observable_value
  ...
  return (
    <div data-testid="artifact-page">
      <ExportContextProvider>
        <Breadcrumbs elements={[{ label: 'Observations' }, { label: 'Artifacts', current: true }]} />
        {queryRef && (
          <DataTable
            dataColumns={dataColumns}
            resolvePath={(data: ArtifactsLines_data$data) =>
              data.stixCyberObservables?.edges?.map((n: { node: any }) => n?.node)
            }
            ...
            createButton={<ArtifactCreation paginationOptions={queryPaginationOptions} />}
          />
        )}
      </ExportContextProvider>
    </div>
