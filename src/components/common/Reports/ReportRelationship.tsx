...
interface ReportRelationshipProps {
  reportId?: string;
}

export const ReportRelationship: React.FC<ReportRelationshipProps> = ({ reportId }) => {
  const { id: paramId } = useParams<{ id: string }>();
  const id = reportId || paramId;
  const navigate = useNavigate();

  // Fetch relationships where report is source
  const {
  ...
  // Fetch relationships where report is target
  const {
  ...
  React.useEffect(() => {
    if (error) {
      ...
      console.error('ReportRelationship error:', error);
    }
  }, [error]);

  if (!id) {
    return <div className="p-4 text-red-500">No report ID provided.</div>;
  }
  ...
  if (!relationships || relationships.length === 0) {
    return <div className="p-4 text-gray-500">No relationships found for this report.</div>;
  }
...
export default ReportRelationship;
