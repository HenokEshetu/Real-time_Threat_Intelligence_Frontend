...
export interface ReportCardProps {
  report: {
    ...
    modified?: string;
    labels?: string[];
    external_references?: { id?: string; source_name: string; url?: string; external_id?: string }[];
  };
    ...
    >
        ...
        <div className="flex-1">
          ...
          <div className="flex flex-wrap gap-1 mt-1">
            {report.report_types?.map((type) => (
              <span key={type} className="bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-200 px-2 py-0.5 rounded text-xs font-medium">{type}</span>
            ))}
            {report.labels?.map((label) => (
              <span key={label} className="bg-zinc-200 text-zinc-700 dark:bg-zinc-800 dark:text-zinc-200 px-2 py-0.5 rounded text-xs">{label}</span>
            ))}
          </div>
        </div>
      </div>
      <div className="px-5 pb-2 text-sm text-zinc-700 dark:text-zinc-300 line-clamp-2">{report.description}</div>
      <div className="px-5 pb-2 text-xs text-zinc-500 dark:text-zinc-400 flex flex-wrap gap-2">
        {report.published && <span>Published: <span className="font-semibold">{new Date(report.published).toLocaleDateString()}</span></span>}
        {report.confidence !== undefined && (
          <span>
            Confidence:{" "}
            <span className={report.confidence > 80 ? "text-green-600" : report.confidence > 50 ? "text-yellow-600" : "text-red-600"}>
              {report.confidence}
            </span>
          </span>
        )}
      ...
      {expanded && (
        <div className="px-5 pb-5 text-xs text-zinc-600 dark:text-zinc-400 space-y-1">
          ...
          {report.created && (
            <div>
              <span className="font-medium">Created:</span> {new Date(report.created).toLocaleString()}
            </div>
          )}
          {report.modified && (
            <div>
              <span className="font-medium">Modified:</span> {new Date(report.modified).toLocaleString()}
            </div>
          )}
              ...
              <ul className="list-disc ml-5">
                {report.external_references.map((ref, idx) => (
                  <li key={ref.id ?? ref.url ?? ref.source_name ?? idx}>
                    ...
                    {ref.external_id && <span className="ml-1 text-zinc-400">[{ref.external_id}]</span>}
                  </li>
                ))}
              </ul>
            </div>
