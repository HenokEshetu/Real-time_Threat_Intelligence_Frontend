...
interface CourseOfActionRelationshipsProps {
  courseOfActionId?: string;
}

const CourseOfActionRelationships: React.FC<CourseOfActionRelationshipsProps> = ({ courseOfActionId }) => {
  const { id: paramId } = useParams<{ id: string }>();
  const id = courseOfActionId || paramId;
  const navigate = useNavigate();

  // Fetch relationships where course of action is source
  const {
  ...
  // Fetch relationships where course of action is target
  const {
  ...
  if (!id) {
    return <div className="p-4 text-red-500">No course of action ID provided.</div>;
  }
  ...
  if (!relationships || relationships.length === 0) {
    return <div className="p-4 text-gray-500">No relationships found for this course of action.</div>;
  }
  ...
  const getDetailRoute = (obj: any) => {
    ...
    switch (obj.type) {
      ...
      case 'course-of-action': return `/courseofaction/${obj.id}`;
      // Add more as needed
      default: return null;
    }
  };
...
export default CourseOfActionRelationships;
