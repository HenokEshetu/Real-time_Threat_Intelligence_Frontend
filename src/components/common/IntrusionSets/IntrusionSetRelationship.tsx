...
interface IntrusionSetRelationshipProps {
  intrusionSetId?: string;
}

const IntrusionSetRelationship: React.FC<IntrusionSetRelationshipProps> = ({ intrusionSetId }) => {
  const { id: paramId } = useParams<{ id: string }>();
  const id = intrusionSetId || paramId;
  const navigate = useNavigate();

  // Fetch relationships where intrusion set is source
  const {
  ...
  // Fetch relationships where intrusion set is target
  const {
  ...
  if (!id) {
    return <div className="p-4 text-red-500">No intrusion set ID provided.</div>;
  }
  ...
  if (!relationships || relationships.length === 0) {
    return <div className="p-4 text-gray-500">No relationships found for this intrusion set.</div>;
  }
  ...
  const getDetailRoute = (obj: any) => {
    ...
    switch (obj.type) {
      ...
      case 'intrusion-set': return `/intrusionsets/${obj.id}`;
      // Add more as needed
      default: return null;
    }
  };
      ...
      <table className="min-w-full text-sm border">
        ...
        <tbody>
          {relationships.map((rel: StixRelationship) => {
            // Cast to any to access source/target added by the hook
            const from = (rel as any).source;
            const to = (rel as any).target;
...
export default IntrusionSetRelationship;
