...
export const HexagonToolCard: React.FC<HexagonToolCardProps> = ({
  tool,
  onClick,
  className,
}) => (
  <div
    className={`
      w-[340px] h-[340px] flex items-center justify-center
      bg-gray-400
      m-2
      shadow-xl
  ...
  >
    <div className="flex flex-col items-center justify-center h-full w-full p-8 bg-white/80 rounded-[2rem]">
      ...
      <div className="font-bold text-blue-900 text-center text-xl truncate">
        {tool.name}
      </div>
      ...
      <div className="flex flex-wrap gap-2 justify-center mt-4">
        {(tool.labels || []).slice(0, 2).map((label) => (
          <Badge
            key={label}
            className="text-sm bg-blue-100 text-blue-700 border-none"
          >
            {label}
          </Badge>
        ))}
