...
const LocateControl = () => {
  ...
  return (
    <div className="leaflet-control-locate leaflet-bar leaflet-control" onClick={handleLocate}>
...
const FullscreenControl = () => {
  ...
  return (
    <div className="leaflet-control-fullscreen leaflet-bar leaflet-control" onClick={toggleFullscreen}>
  ...
  const handleSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (query.length > 1) {
      const results = markers.filter(marker => 
        marker.content.toLowerCase().includes(query)
      ).slice(0, 5);
  ...
  return (
    <div className="leaflet-control-search leaflet-bar leaflet-control">
      {isSearchOpen ? (
        ...
        <a href="#" title="Search locations" onClick={() => setIsSearchOpen(true)}>
          <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    ...
    <div style={{ position: 'relative', width, height }}>
              ...
              <Circle 
                ...
                pathOptions={{ 
                  color: marker.type === 'warning' ? '#ff9e00' : 
                         marker.type === 'important' ? '#e63946' : 
                         marker.type === 'info' ? '#06d6a0' : '#4361ee',
                  fillOpacity: 0.2,
                  weight: 1
                }} 
              />
      ...
      <style>{`
        ...
        .map-legend {
          ...
          bottom: 20px;
          left: 10px;
          background-color: ${darkMode ? 'rgba(45, 55, 72, 0.9)' : 'rgba(255, 255, 255, 0.9)'};
          padding: 10px;
          border-radius: 8px;
