...
const IndicatorRelationships: React.FC<IndicatorRelationshipsProps> = ({ indicatorId }) => {
  const { id: paramId } = useParams<{ id: string }>();
  const id = indicatorId || paramId;
  const navigate = useNavigate();

  // Fetch relationships where indicator is source
  const {
  ...
  // Fetch relationships where indicator is target
  const {
  ...
  if (!id) {
    return <div className="p-4 text-red-500">No indicator ID provided.</div>;
  }
  ...
  if (!relationships || relationships.length === 0) {
    return <div className="p-4 text-gray-500">No relationships found for this indicator.</div>;
  }
