...
export const EmailMessageOverview = ({ emailMessage }: { emailMessage: EmailMessage }) => {
  const labels: string[] = emailMessage.labels || [];
        ...
        <Card className="bg-transparent border border-gray-300 rounded-sm shadow-none !py-4">
          <CardContent className="px-3">
            <div className="pb-3 w-full">
              <h2 className="font-semibold text-base mb-2">Subject</h2>
            ...
            <div className="flex justify-between py-3">
              <div className="w-[48%]">
                <h2 className="font-bold text-base mb-2">From</h2>
                <p className="bg-slate-100 border border-slate-600 text-slate-600 font-semibold p-2 rounded w-full font-mono text-sm text-center">
                  {emailMessage.from_ref}
                </p>
              </div>
              <div className="w-[48%]">
                <h2 className="font-bold text-base mb-2">To</h2>
                <p className="bg-slate-100 border border-slate-600 text-slate-600 font-semibold p-2 rounded w-full font-mono text-sm text-center">
                  {(emailMessage.to_refs && emailMessage.to_refs.join(', ')) || '—'}
                </p>
              </div>
            </div>
            <div className="flex justify-between py-3">
              <div className="w-[31%]">
                ...
                <span
                  className={`${getConfidenceColor(emailMessage.confidence || 0)} py-1 px-6 rounded text-sm text-center uppercase`}
                >
                  {emailMessage.confidence || 0}%
                </span>
              </div>
              <div className="w-[31%]">
                <h2 className="font-bold text-base mb-2">Is Multipart</h2>
                <span className="bg-blue-100 py-1 px-7 rounded text-sm text-center text-blue-800 uppercase">
                  {emailMessage.is_multipart ? 'Yes' : 'No'}
                </span>
              </div>
              <div className="w-[31%]">
            ...
            <div className="flex justify-between py-5">
              {emailMessage.external_references && (
                <div className="w-full">
                  ...
                  <table className="w-full text-sm text-foreground">
                    <tbody>
                      {emailMessage.external_references.map((ref, index) => (
                        <tr
                          key={index}
      ...
      <div>
        ...
        <Card className="bg-transparent shadow-none border border-gray-300 rounded-sm">
          <CardContent className="space-y-4 p-6">
            <div className="flex justify-between py-3">
              <div className="w-[48%]">
                <h2 className="font-bold text-sm mb-2">Message ID</h2>
                <span className="bg-blue-100 text-blue-800 border border-blue-800 py-1 px-5 rounded text-sm text-center uppercase">
                  {emailMessage.message_id || emailMessage.id}
                </span>
              </div>
              <div className="w-[48%]">
                <h2 className="font-bold text-sm mb-2">Created By</h2>
                <span className="bg-slate-200 p-2 rounded text-sm uppercase">
                  {emailMessage.created_by_ref}
                </span>
              </div>
            </div>
            ...
            <div className="flex justify-between py-2">
              <div className="w-full">
                ...
                <div className="bg-slate-200 p-2 rounded text-sm uppercase">
                  {emailMessage.modified
                    ? new Date(emailMessage.modified).toLocaleDateString()
                    : '—'}
                </div>
              </div>
            </div>
            <div className="space-y-1 text-sm">
              <div>
                ...
                {emailMessage.created
                  ? new Date(emailMessage.created).toLocaleString()
                  : '—'}
              </div>
              <div>
                <strong>Modified:</strong>{' '}
                {emailMessage.modified
                  ? new Date(emailMessage.modified).toLocaleString()
                  : '—'}
              </div>
            </div>
          </CardContent>
