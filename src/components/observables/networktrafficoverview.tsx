...
export const NetworkTrafficOverview = ({ networkTraffic }: { networkTraffic: NetworkTraffic }) => {
  const labels: string[] = networkTraffic.labels || [];
        ...
        <Card className="bg-transparent border border-gray-300 rounded-sm shadow-none !py-4">
          <CardContent className="px-3">
            <div className="pb-3 w-full">
              <h2 className="font-semibold text-base mb-2">ID</h2>
            ...
            <div className="flex justify-between py-3">
              <div className="w-[31%]">
                ...
                <span
                  className={`${getConfidenceColor(networkTraffic.confidence || 0)} py-1 px-6 rounded text-sm text-center uppercase`}
                >
                  {networkTraffic.confidence || 0}%
                </span>
              </div>
              <div className="w-[31%]">
                <h2 className="font-bold text-base mb-2">Active</h2>
                <span className={`py-1 px-7 rounded text-sm text-center uppercase ${networkTraffic.is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                  {networkTraffic.is_active ? 'Yes' : 'No'}
                </span>
              </div>
              <div className="w-[31%]">
            ...
            <div className="flex justify-between py-5">
              {networkTraffic.external_references && (
                <div className="w-full">
                  ...
                  <table className="w-full text-sm text-foreground">
                    <tbody>
                      {networkTraffic.external_references.map((ref, index) => (
                        <tr
                          key={index}
          ...
          <CardContent className="space-y-4 p-6">
              ...
              <div className="w-[48%]">
                ...
                <span
                  className={`py-1 px-5 rounded text-sm text-center uppercase border ${getTlpColors(
                    (networkTraffic.object_marking_refs && networkTraffic.object_marking_refs[0]) || 'clear',
                  )}`}
                >
                  TLP:{(networkTraffic.object_marking_refs && networkTraffic.object_marking_refs[0]) || 'clear'}
                </span>
              </div>
            </div>
            ...
            <div className="flex justify-between py-2">
              <div className="w-full">
                ...
                <div className="bg-slate-200 p-2 rounded text-sm uppercase">
                  {new Date(networkTraffic.modified).toLocaleDateString()}
                </div>
              </div>
            </div>
            <div className="space-y-1 text-sm">
              <div>
                ...
                {new Date(networkTraffic.created).toLocaleString()}
              </div>
              <div>
                <strong>Modified:</strong>{' '}
                {new Date(networkTraffic.modified).toLocaleString()}
              </div>
            </div>
          </CardContent>
        </Card>
