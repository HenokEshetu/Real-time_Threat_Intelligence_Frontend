...
import { UserAccount } from '@/types/observables/useraccount';
...
export const UserAccountOverview = ({ userAccount }: { userAccount: UserAccount }) => {
  const labels: string[] = userAccount.labels || [];
        ...
        <Card className="bg-transparent border border-gray-300 rounded-sm shadow-none !py-4">
          <CardContent className="px-3">
            <div className="pb-3 w-full">
              <h2 className="font-semibold text-base mb-2">Account Login</h2>
              <div className="bg-slate-100 border border-slate-600 text-slate-600 font-semibold p-3 rounded overflow-x-auto font-mono text-sm">
                {userAccount.account_login || userAccount.display_name || userAccount.user_id || userAccount.id}
              </div>
            </div>
            ...
            <div className="flex justify-between py-3">
              <div className="w-[31%]">
                ...
                <span
                  className={`${getConfidenceColor(userAccount.confidence || 0)} py-1 px-6 rounded text-sm text-center uppercase`}
                >
                  {userAccount.confidence || 0}%
                </span>
              </div>
              <div className="w-[31%]">
              ...
              <div className="w-[31%]">
                <h2 className="font-bold text-base mb-2">Disabled</h2>
                <span className={`py-1 px-7 rounded text-sm text-center uppercase ${userAccount.is_disabled ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'}`}>
                  {userAccount.is_disabled ? 'Yes' : 'No'}
                </span>
              </div>
            </div>
            <hr />
            <div className="flex justify-between py-5">
              {userAccount.external_references && (
                <div className="w-full">
                  ...
                  <table className="w-full text-sm text-foreground">
                    <tbody>
                      {userAccount.external_references.map((ref, index) => (
                        <tr
                          key={index}
      ...
      <div>
        ...
        <Card className="bg-transparent shadow-none border border-gray-300 rounded-sm">
          <CardContent className="space-y-4 p-6">
            <div className="flex justify-between py-3">
              <div className="w-[48%]">
                <h2 className="font-bold text-sm mb-2">User ID</h2>
              ...
              <div className="w-[48%]">
                ...
                <span
                  className={`py-1 px-5 rounded text-sm text-center uppercase border ${getTlpColors(
                    userAccount.object_marking_refs?.[0] || 'clear',
                  )}`}
                >
                  TLP:{userAccount.object_marking_refs?.[0] || 'clear'}
                </span>
              </div>
            </div>
            ...
            <div className="flex justify-between py-2">
              <div className="w-full">
                ...
                <div className="bg-slate-200 p-2 rounded text-sm uppercase">
                  {new Date(userAccount.modified).toLocaleDateString()}
                </div>
              </div>
            </div>
            <div className="space-y-1 text-sm">
              <div>
                ...
                {new Date(userAccount.created).toLocaleString()}
              </div>
              <div>
                <strong>Modified:</strong>{' '}
                {new Date(userAccount.modified).toLocaleString()}
              </div>
            </div>
          </CardContent>
        </Card>
