...
export const X509CertificateOverview = ({
  certificate,
}: {
  certificate: X509Certificate;
}) => {
  const labels: string[] = certificate.labels || [];
          ...
          <CardContent className="px-3">
            ...
            <div className="flex justify-between py-3">
              ...
              <div className="w-[31%]">
                ...
                <span className={`py-1 px-7 rounded text-sm text-center uppercase ${certificate.revoked ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'}`}>
            ...
            <div className="flex justify-between py-5">
              {certificate.external_references && (
                <div className="w-full">
                  ...
                  <table className="w-full text-sm text-foreground">
                    <tbody>
                      {certificate.external_references.map((ref, index) => (
                        <tr
                          key={index}
            ...
            <div className="py-3">
              <h2 className="font-bold text-base mb-2">Issuer</h2>
              <div className="bg-gray-200 p-2 rounded text-sm font-mono">
                {certificate.issuer}
              </div>
            </div>
            <div className="py-3">
              <h2 className="font-bold text-base mb-2">Serial Number</h2>
              <div className="bg-gray-200 p-2 rounded text-sm font-mono">
                {certificate.serial_number}
              </div>
            </div>
            <div className="py-3">
              <h2 className="font-bold text-base mb-2">Signature Algorithm</h2>
              <div className="bg-gray-200 p-2 rounded text-sm font-mono">
                {certificate.signature_algorithm}
              </div>
            </div>
            <div className="py-3">
              <h2 className="font-bold text-base mb-2">Validity</h2>
              <div className="bg-gray-200 p-2 rounded text-sm font-mono">
                {certificate.validity_not_before} ~ {certificate.validity_not_after}
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="space-y-4 p-6">
              ...
              <div className="w-[48%]">
                ...
                <span
                  className={`py-1 px-5 rounded text-sm text-center uppercase border ${getTlpColors(
                    certificate.object_marking_refs?.[0] || 'clear',
                  )}`}
                >
                  TLP:{certificate.object_marking_refs?.[0] || 'clear'}
                </span>
              </div>
            </div>
            ...
            <div className="space-y-1 text-sm">
              ...
              <div>
                ...
                {new Date(certificate.modified).toLocaleString()}
              </div>
            </div>
          </CardContent>
        </Card>
