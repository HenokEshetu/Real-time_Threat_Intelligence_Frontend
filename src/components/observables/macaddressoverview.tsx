...
export const MACAddressOverview = ({ mac }: { mac: MACAddress }) => {
  const labels: string[] = mac.labels || [];
        ...
        <Card className="bg-transparent border border-gray-300 rounded-sm shadow-none !py-4">
          <CardContent className="px-3">
            <div className="pb-3 w-full">
              <h2 className="font-semibold text-base mb-2">MAC Address</h2>
            ...
            <div className="flex justify-between py-3">
              <div className="w-[31%]">
                ...
                <span
                  className={`${getConfidenceColor(mac.confidence || 0)} py-1 px-6 rounded text-sm text-center uppercase`}
                >
                  {mac.confidence || 0}%
                </span>
              </div>
              <div className="w-[31%]">
                ...
                <span className={`py-1 px-7 rounded text-sm text-center uppercase ${mac.defanged ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                  {mac.defanged ? 'Yes' : 'No'}
                </span>
              </div>
              <div className="w-[31%]">
              ...
              {mac.external_references && (
                <div className="w-full">
                  ...
                  <table className="w-full text-sm text-foreground">
                    <tbody>
                      {mac.external_references.map((ref, index) => (
                        <tr
                          key={index}
          ...
          <CardContent className="space-y-4 p-6">
              ...
              <div className="w-[48%]">
                ...
                <span
                  className={`py-1 px-5 rounded text-sm text-center uppercase border ${getTlpColors(
                    mac.object_marking_refs?.[0] || 'clear',
                  )}`}
                >
                  TLP:{mac.object_marking_refs?.[0] || 'clear'}
                </span>
              </div>
            </div>
            ...
            <div className="space-y-1 text-sm">
              <div>
                ...
                {new Date(mac.created).toLocaleString()}
              </div>
              <div>
                <strong>Modified:</strong>{' '}
                {new Date(mac.modified).toLocaleString()}
              </div>
            </div>
          </CardContent>
        </Card>
