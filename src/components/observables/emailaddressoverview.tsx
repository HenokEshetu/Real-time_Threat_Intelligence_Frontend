...
import { EmailAddress } from '@/types/observables/emailaddress';
...
export const EmailAddressOverview = ({ email }: { email: EmailAddress }) => {
  const labels: string[] = email.labels || [];
        ...
        <Card className="bg-transparent border border-gray-300 rounded-sm shadow-none !py-4">
          <CardContent className="px-3">
            <div className="pb-3 w-full">
              <h2 className="font-semibold text-base mb-2">Email Address</h2>
            ...
            <div className="flex justify-between py-3">
              ...
              <div className="w-[31%]">
                ...
                <span className={`py-1 px-7 rounded text-sm text-center uppercase ${email.defanged ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                  {email.defanged ? 'Yes' : 'No'}
                </span>
              </div>
              <div className="w-[31%]">
              ...
              {email.external_references && (
                <div className="w-full">
                  ...
                  <table className="w-full text-sm text-foreground">
                    <tbody>
                      {email.external_references.map((ref, index) => (
                        <tr
                          key={index}
      ...
      <div>
        ...
        <Card className="bg-transparent shadow-none border border-gray-300 rounded-sm">
          <CardContent className="space-y-4 p-6">
            <div className="flex justify-between py-3">
              <div className="w-[48%]">
                <h2 className="font-bold text-sm mb-2">Display Name</h2>
              ...
              <div className="w-[48%]">
                ...
                <span
                  className={`py-1 px-5 rounded text-sm text-center uppercase border ${getTlpColors(
                    email.object_marking_refs[0] || 'clear',
                  )}`}
                >
                  TLP:{email.object_marking_refs[0] || 'clear'}
                </span>
              </div>
            </div>
            ...
            <div className="flex justify-between py-2">
              <div className="w-full">
                ...
                <div className="bg-slate-200 p-2 rounded text-sm uppercase">
                  {new Date(email.modified).toLocaleDateString()}
                </div>
              </div>
            </div>
            <div className="space-y-1 text-sm">
              ...
              <div>
                ...
                {new Date(email.modified).toLocaleString()}
              </div>
            </div>
          </CardContent>
        </Card>
