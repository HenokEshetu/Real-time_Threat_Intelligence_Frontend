...
import { useUserAccount, useUpdateUserAccount } from '@/hooks/observables/useUserAccount';
...
export const UserAccountObservablesEditPage = () => {
  const navigate = useNavigate();
  const { id } = useParams<{ id: string }>();
  const { account, loading, error } = useUserAccount(id || '');
  const [updateUserAccount, { loading: updating, error: updateError }] = useUpdateUserAccount();
  ...
  useEffect(() => {
    if (account) {
      setValues({
        ...
        confidence: typeof account.confidence === 'number' ? account.confidence : 100,
        user_id: account.user_id || '',
        markingLabels: Array.isArray(account.object_marking_refs) && account.object_marking_refs.length > 0 ? account.object_marking_refs[0] : '',
      });
    }
  }, [account]);
  ...
  if (error || !account) {
    return (
      <div className="min-h-screen flex items-center justify-center text-destructive">
        Failed to load user account observable.
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-muted flex flex-col">
      <div className="w-full bg-background border-b px-4 py-3 flex items-center">
        ...
        <h1 className="text-lg font-semibold">Update a user account observable</h1>
      </div>
