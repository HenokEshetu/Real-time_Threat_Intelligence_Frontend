...
import { useUserAccounts } from '@/hooks/observables/useUserAccount';
...
export const UserAccountObservablesPage = () => {
  const navigate = useNavigate();
  const [currentPage, setCurrentPage] = useState(1);
  const pageSize = 50;

  const { loading, error, total, accounts: userAccounts } = useUserAccounts({
    filters: {},
    page: currentPage,
    pageSize,
  });
  ...
  const handleViewUserAccount = (id: string) => navigate(`/observables/useraccounts/${id}`);
    ...
    <div className="w-full flex flex-col space-y-4">
      <div className="overflow-x-auto overflow-y-auto h-[81vh] bg-white rounded-md">
        <Table className="w-full text-sm text-foreground h-full">
          ...
          <TableBody>
            {userAccounts.map((userAccount: UserAccount) => {
              const labels = userAccount.labels || [];
              ...
              return (
                <TableRow
                  key={userAccount.id}
                  onClick={() => handleViewUserAccount(userAccount.id)}
                ...
                >
                    ...
                    <Badge
                      ...
                      className={`max-w-28 ${getTlpColors(
                        marking.replaceAll('tlp:', '') ||
                          userAccount.object_marking_refs?.[0],
                      )} border-2 uppercase truncate`}
                    >
                      {marking || `TLP:${userAccount.object_marking_refs?.[0]}`}
                    </Badge>
                  </TableCell>
                  ...
                  <TableCell className="p-4 text-gray-600">
                    {userAccount.created
                      ? new Date(userAccount.created).toLocaleDateString()
                      : '—'}
                  </TableCell>
                  <TableCell className="p-4 text-gray-600">
      ...
      <div className="flex items-center justify-between px-4 py-2 border-t">
        <div className="text-sm text-muted-foreground">
          ...
          {Math.min(currentPage * pageSize, total || 0)} of {total || 0} user accounts
        </div>
