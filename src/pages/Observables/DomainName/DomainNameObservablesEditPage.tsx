...
import { useDomainName, useUpdateDomainName } from '@/hooks/observables/useDomains';
...
export const DomainNameObservablesEditPage = () => {
  const navigate = useNavigate();
  const { id } = useParams<{ id: string }>();
  const { domainName, loading, error } = useDomainName(id || '');
  const { updateDomainName, loading: updating, error: updateError } = useUpdateDomainName();
  ...
  useEffect(() => {
    if (domainName) {
      setValues({
        ...
        type: domainName.type || 'domain-name',
        confidence: typeof domainName.confidence === 'number' ? domainName.confidence : 100,
        description: '', // You may want to fetch description if available
        markingLabels: Array.isArray(domainName.object_marking_refs) && domainName.object_marking_refs.length > 0 ? domainName.object_marking_refs[0] : '',
      });
    }
  }, [domainName]);
  ...
  if (error || !domainName) {
    return (
      <div className="min-h-screen flex items-center justify-center text-destructive">
        Failed to load domain name observable.
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-muted flex flex-col">
      <div className="w-full bg-background border-b px-4 py-3 flex items-center">
        ...
        <h1 className="text-lg font-semibold">Update a domain name observable</h1>
      </div>
