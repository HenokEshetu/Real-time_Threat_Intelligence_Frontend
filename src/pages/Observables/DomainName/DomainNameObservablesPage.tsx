...
import { useDomains } from '@/hooks/observables/useDomains';
...
export const DomainNameObservablesPage = () => {
  const navigate = useNavigate();
  const [currentPage, setCurrentPage] = useState(1);
  const pageSize = 50;

  const { loading, error, loadMore, total, domains } = useDomains({
    filter: {},
    page: currentPage,
    pageSize: pageSize,
  });
  ...
  const handleViewDomain = (id: string) =>
    navigate(`/observables/domain-names/${id}`);
    ...
    return (
      <div className="container py-6">
        <Alert variant="destructive">
          <AlertDescription>
            Error loading domain names: {error.message}
          </AlertDescription>
        </Alert>
      </div>
    );
    ...
    <div className="w-full flex flex-col space-y-4">
              ...
              return (
                ...
                >
                  <TableCell className="p-4 text-gray-700">
                    <Badge
                      variant="outline"
                      className="text-indigo-600 border-indigo-500 bg-indigo-50 capitalize px-8"
                    >
                      <GlobeIcon />
                      {domain.type.replace('-', ' ')}
                    </Badge>
                  ...
                  <TableCell className="p-4 text-gray-600">
                    {domain.created
                      ? new Date(domain.created).toLocaleDateString()
                      : '—'}
                  </TableCell>
                  <TableCell className="p-4 text-gray-600">
                    {domain.modified
      ...
      <div className="flex items-center justify-between px-4 py-2 border-t">
        <div className="text-sm text-muted-foreground">
          ...
          {Math.min(currentPage * pageSize, total || 0)} of {total || 0} domain
          names
        </div>
