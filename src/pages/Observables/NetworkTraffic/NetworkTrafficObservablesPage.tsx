...
import { useNetworkTraffic } from '@/hooks/observables/useNetworkTraffic';
...
export const NetworkTrafficObservablesPage = () => {
  ...
  const { loading, error, total, networkTraffic } = useNetworkTraffic({
    filter: {},
  ...
  const handleView = (id: string) => navigate(`/observables/network-traffic/${id}`);
  ...
  if (loading && networkTraffic.length === 0) {
    return (
    ...
    return (
      <div className="container py-6">
        <Alert variant="destructive">
          <AlertDescription>
            Error loading network traffic: {error.message}
          </AlertDescription>
        </Alert>
      </div>
    );
    ...
    <div className="w-full flex flex-col space-y-4">
      <div className="overflow-x-auto overflow-y-auto h-[81vh] bg-white rounded-md">
        <Table className="w-full text-sm text-foreground h-full">
          ...
          <TableBody>
            {networkTraffic.map((nt: NetworkTraffic) => {
              ...
              return (
                ...
                >
                    ...
                    <Badge
                      ...
                      className={`max-w-28 ${getTlpColors(
                        marking.replaceAll('tlp:', '') ||
                          (nt.object_marking_refs && nt.object_marking_refs[0]),
                      )} border-2 uppercase truncate`}
                    >
                      {marking || `TLP:${nt.object_marking_refs && nt.object_marking_refs[0]}`}
                    </Badge>
                  </TableCell>
                  ...
                  <TableCell className="p-4 text-gray-600">
                    {nt.created
                      ? new Date(nt.created).toLocaleDateString()
                      : '—'}
                  </TableCell>
                  <TableCell className="p-4 text-gray-600">
                    {nt.modified
                      ? new Date(nt.modified).toLocaleDateString()
                      : '—'}
                  </TableCell>
                </TableRow>
              );
      ...
      <div className="flex items-center justify-between px-4 py-2 border-t">
        <div className="text-sm text-muted-foreground">
          ...
          {Math.min(currentPage * pageSize, total || 0)} of {total || 0} network traffic
        </div>
