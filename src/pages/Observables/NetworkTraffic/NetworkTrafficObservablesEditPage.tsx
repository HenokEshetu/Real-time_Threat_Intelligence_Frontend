...
import { useNetworkTrafficById, useUpdateNetworkTraffic } from '@/hooks/observables/useNetworkTraffic';
...
export const NetworkTrafficObservablesEditPage = () => {
  const navigate = useNavigate();
  const { id } = useParams<{ id: string }>();
  const { networkTraffic, loading, error } = useNetworkTrafficById(id || '');
  const [updateNetworkTraffic, { loading: updating, error: updateError }] = useUpdateNetworkTraffic();
  ...
  useEffect(() => {
    if (networkTraffic && networkTraffic.id) {
      setValues({
        confidence: typeof networkTraffic.confidence === 'number' ? networkTraffic.confidence : 100,
        protocols: Array.isArray(networkTraffic.protocols) ? networkTraffic.protocols : [],
        description: '', // Add description if available in schema
        markingLabels: Array.isArray(networkTraffic.object_marking_refs) && networkTraffic.object_marking_refs.length > 0 ? networkTraffic.object_marking_refs[0] : '',
      });
    }
  }, [networkTraffic]);
  ...
  if (error || !networkTraffic || !networkTraffic.id) return <ErrorMessage message="Failed to load network traffic observable." />;
  ...
  return (
    <div className="min-h-screen bg-muted flex flex-col">
      <div className="w-full bg-background border-b px-4 py-3 flex items-center">
        ...
        <h1 className="text-lg font-semibold">Update a network traffic observable</h1>
      </div>
