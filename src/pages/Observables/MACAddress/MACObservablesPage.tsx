...
import { useMacAddresses } from '@/hooks/observables/useMacAddress';
...
export const MACObservablesPage = () => {
  ...
  const handleView = (id: string) => navigate(`/observables/macaddress/${id}`);
  ...
  if (loading && macAddresses.length === 0) {
    return (
    ...
    return (
      <div className="container py-6">
        <Alert variant="destructive">
          <AlertDescription>
            Error loading MAC addresses: {error.message}
          </AlertDescription>
        </Alert>
      </div>
    );
    ...
    <div className="w-full flex flex-col space-y-4">
        ...
        <Table className="w-full text-sm text-foreground h-full">
          <TableHeader>
            <TableRow className="bg-gray-100">
              ...
              <TableHead className="font-bold p-4 text-gray-800">MAC Address</TableHead>
              ...
              return (
                ...
                >
                  <TableCell className="p-4 text-gray-700">
                    ...
                    >
                      <NetworkIcon className="inline mr-1" />
                      {mac.type}
                    </Badge>
                  </TableCell>
                    ...
                    <Badge
                      ...
                      className={`max-w-28 ${getTlpColors(
                        marking.replaceAll('tlp:', '') ||
                          mac.object_marking_refs?.[0],
                      )} border-2 uppercase truncate`}
                    >
                      {marking || `TLP:${mac.object_marking_refs?.[0]}`}
                    </Badge>
                  </TableCell>
                  ...
                  <TableCell className="p-4 text-gray-600">
                    {mac.created
                      ? new Date(mac.created).toLocaleDateString()
                      : '—'}
                  </TableCell>
                  <TableCell className="p-4 text-gray-600">
                    {mac.modified
                      ? new Date(mac.modified).toLocaleDateString()
                      : '—'}
                  </TableCell>
                </TableRow>
              );
      ...
      <div className="flex items-center justify-between px-4 py-2 border-t">
        <div className="text-sm text-muted-foreground">
          ...
          {Math.min(currentPage * pageSize, total || 0)} of {total || 0} MAC addresses
        </div>
