...
// import { useUpdateIPv4 } from '@/hooks/observables/useIPv4Adresses';
...
export const IPv4ObservablesEditPage = () => {
  const navigate = useNavigate();
  const { id } = useParams<{ id: string }>();
  const { ipv4, loading, error } = useIPv4Address(id);
  // const [updateIPv4, { loading: updating, error: updateError }] = useUpdateIPv4();
  ...
  useEffect(() => {
    if (ipv4) {
      setValues({
        value: ipv4.value || '',
        type: ipv4.type || 'ipv4-addr',
        confidence: typeof ipv4.confidence === 'number' ? ipv4.confidence : 100,
        defanged: !!ipv4.defanged,
        markingLabels: Array.isArray(ipv4.object_marking_refs) && ipv4.object_marking_refs.length > 0 ? ipv4.object_marking_refs[0] : '',
        labels: Array.isArray(ipv4.labels) ? ipv4.labels.join(', ') : '',
      });
    }
  }, [ipv4]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!validate()) return;
    // await updateIPv4({ variables: { id, ...values, labels: values.labels.split(',').map(l => l.trim()).filter(Boolean), object_marking_refs: [values.markingLabels] } });
  ...
  if (error || !ipv4) {
    return (
      <div className="min-h-screen flex items-center justify-center text-destructive">
        Failed to load IPv4 observable.
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-muted flex flex-col">
      <div className="w-full bg-background border-b px-4 py-3 flex items-center">
        ...
        <h1 className="text-lg font-semibold">Update an IPv4 observable</h1>
      </div>
