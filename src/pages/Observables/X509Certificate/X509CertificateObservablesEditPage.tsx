...
import { useX509Certificate, useUpdateX509Certificate } from '@/hooks/observables/useX509certificate';
...
export const X509CertificateObservablesEditPage = () => {
  const navigate = useNavigate();
  const { id } = useParams<{ id: string }>();
  const { certificate, loading, error } = useX509Certificate(id || '');
  const [updateCertificate, { loading: updating, error: updateError }] = useUpdateX509Certificate();
  ...
  useEffect(() => {
    if (certificate) {
      setValues({
        ...
        confidence: typeof certificate.confidence === 'number' ? certificate.confidence : 100,
        version: certificate.version || '',
        markingLabels: Array.isArray(certificate.object_marking_refs) && certificate.object_marking_refs.length > 0 ? certificate.object_marking_refs[0] : '',
      });
    }
  }, [certificate]);
  ...
  if (error || !certificate) return <ErrorMessage message={error?.message || 'Certificate not found.'} />;
  ...
  return (
    <div className="min-h-screen bg-muted flex flex-col">
      <div className="w-full bg-background border-b px-4 py-3 flex items-center">
        ...
        <h1 className="text-lg font-semibold">Update an X509 Certificate observable</h1>
      </div>
