...
import { useX509Certificates } from '@/hooks/observables/useX509certificate';
import { X509Certificate } from '@/types/observables/x509certificate';
import {
  ChevronLeft,
  ChevronRight,
  ChevronsLeft,
...
export const X509CertificateObservablesPage = () => {
  const navigate = useNavigate();
  const [currentPage, setCurrentPage] = useState(1);
  const pageSize = 50;

  const { loading, error, loadMore, total, certificates } = useX509Certificates({
    filters: {},
  ...
  const handleView = (id: string) => navigate(`/observables/x509-certificates/${id}`);
    ...
    <div className="w-full flex flex-col space-y-4">
        ...
        <Table className="w-full text-sm text-foreground h-full">
          <TableHeader>
            <TableRow className="bg-gray-100">
              <TableHead className="font-bold p-4 text-gray-800">Type</TableHead>
              <TableHead className="font-bold p-4 text-gray-800">Subject</TableHead>
              <TableHead className="font-bold p-4 text-gray-800">Marking</TableHead>
              <TableHead className="font-bold p-4 text-gray-800">Labels</TableHead>
              <TableHead className="font-bold p-4 text-gray-800">Created</TableHead>
              <TableHead className="font-bold p-4 text-gray-800">Modified</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {certificates.map((cert: X509Certificate) => {
              const labels = cert.labels || [];
                    ...
                    <Badge
                      ...
                      className={`max-w-28 ${getTlpColors(
                        marking.replaceAll('tlp:', '') ||
                          cert.object_marking_refs?.[0],
                      )} border-2 uppercase truncate`}
                    >
                      {marking || `TLP:${cert.object_marking_refs?.[0]}`}
                    </Badge>
                  </TableCell>
      ...
      <div className="flex items-center justify-between px-4 py-2 border-t">
        <div className="text-sm text-muted-foreground">
          ...
          {Math.min(currentPage * pageSize, total || 0)} of {total || 0} certificates
        </div>
