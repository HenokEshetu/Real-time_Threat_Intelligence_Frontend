...
import { useEmailAddresses } from '@/hooks/observables/useEmailAddress';
...
// ...tailwindColors and getRandomTailwindColor definitions (same as FileObservablesPage)...
...
export const EmailAddressObservablesPage = () => {
  const navigate = useNavigate();
  const [currentPage, setCurrentPage] = useState(1);
  const pageSize = 50;

  const { loading, error, total, emailAddresses } = useEmailAddresses({
    filter: {},
  ...
  const handleView = (id: string) => navigate(`/observables/emailaddresses/${id}`);
    ...
    return (
      <div className="container py-6">
        <Alert variant="destructive">
          <AlertDescription>
            Error loading email addresses: {error.message}
          </AlertDescription>
        </Alert>
      </div>
    );
    ...
    <div className="w-full flex flex-col space-y-4">
      <div className="overflow-x-auto overflow-y-auto h-[81vh] bg-white rounded-md">
        <Table className="w-full text-sm text-foreground h-full">
          ...
          <TableBody>
            {emailAddresses.map((email: EmailAddress) => {
                    ...
                    <Badge
                      ...
                      className={`max-w-28 ${getTlpColors(
                        marking.replaceAll('tlp:', '') ||
                          email.object_marking_refs[0],
                      )} border-2 uppercase truncate`}
                    >
                      {marking || `TLP:${email.object_marking_refs[0]}`}
                    </Badge>
                  </TableCell>
      ...
      <div className="flex items-center justify-between px-4 py-2 border-t">
        <div className="text-sm text-muted-foreground">
          ...
          {Math.min(currentPage * pageSize, total || 0)} of {total || 0} email addresses
        </div>
