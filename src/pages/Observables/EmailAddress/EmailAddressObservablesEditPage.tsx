...
import { useEmailAddress, useUpdateEmailAddress } from '@/hooks/observables/useEmailAddress';
...
export const EmailAddressObservablesEditPage = () => {
  const navigate = useNavigate();
  const { id } = useParams<{ id: string }>();
  const { email, loading, error } = useEmailAddress(id || '');
  const { updateEmailAddress, loading: updating, error: updateError } = useUpdateEmailAddress();
  ...
  useEffect(() => {
    if (email) {
      setValues({
        ...
        type: email.type || 'email-addr',
        confidence: typeof email.confidence === 'number' ? email.confidence : 100,
        defanged: !!email.defanged,
        markingLabels: Array.isArray(email.object_marking_refs) && email.object_marking_refs.length > 0 ? email.object_marking_refs[0] : '',
      });
    }
  }, [email]);
    ...
    await updateEmailAddress({
      variables: {
        ...
        input: {
          ...
          labels: email.labels || [],
          belongs_to_refs: email.belongs_to_refs || [],
          created: email.created,
          modified: new Date().toISOString(),
          created_by_ref: email.created_by_ref || '',
          enrichment: email.enrichment || {},
          extensions: email.extensions || {},
          external_references: email.external_references || [],
          object_marking_refs: values.markingLabels ? [values.markingLabels] : [],
          spec_version: email.spec_version || '2.1',
        }
      }
    });
  ...
  if (error || !email) {
    return (
      <div className="min-h-screen flex items-center justify-center text-destructive">
        Failed to load email address observable.
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-muted flex flex-col">
      <div className="w-full bg-background border-b px-4 py-3 flex items-center">
        ...
        <h1 className="text-lg font-semibold">Update an email address observable</h1>
      </div>
        ...
        <div className="relative mt-6">
          <div className="border border-input rounded-xl px-4 pt-5 pb-3 bg-background">
            <Input
              ...
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleChange('display_name', e.target.value)}
