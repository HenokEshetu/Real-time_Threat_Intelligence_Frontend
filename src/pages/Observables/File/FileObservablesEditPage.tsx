...
// import { useUpdateFile } from '@/hooks/observables/useFiles';
...
export const FileObservablesEditPage = () => {
  const navigate = useNavigate();
  const { id } = useParams<{ id: string }>();
  const { file, loading, error } = useFile(id || '');
  // const [updateFile, { loading: updating, error: updateError }] = useUpdateFile();
  ...
  useEffect(() => {
    if (file) {
      setValues({
        name: file.name || '',
        type: file.type || '',
        confidence: typeof file.confidence === 'number' ? file.confidence : 100,
        description: '', // You may want to fetch description if available
        markingLabels: Array.isArray(file.object_marking_refs) && file.object_marking_refs.length > 0 ? file.object_marking_refs[0] : '',
      });
    }
  }, [file]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!validate()) return;
    // await updateFile({ variables: { id, ...values } });
  ...
  if (error || !file) {
    return (
      <div className="min-h-screen flex items-center justify-center text-destructive">
        Failed to load file observable.
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-muted flex flex-col">
      <div className="w-full bg-background border-b px-4 py-3 flex items-center">
        ...
        <h1 className="text-lg font-semibold">Update a file observable</h1>
      </div>
