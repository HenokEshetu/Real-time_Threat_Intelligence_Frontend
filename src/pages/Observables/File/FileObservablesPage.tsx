...
export const FileObservablesPage = () => {
  const navigate = useNavigate();
  const [currentPage, setCurrentPage] = useState(1);
  const pageSize = 50;

  const { loading, error, loadMore, total, files } = useFiles({
    filter: {},
  ...
  const handleViewFile = (id: string) => navigate(`/observables/files/${id}`);
    ...
    <div className="w-full flex flex-col space-y-4">
              ...
              return (
                ...
                >
                  <TableCell className="p-4 text-gray-700">
                    <Badge
                      variant="outline"
                      className="text-lime-600 border-lime-500 bg-lime-50 capitalize px-8"
                    >
                      <FileTextIcon />
                      {file.type}
                    </Badge>
      ...
      <div className="flex items-center justify-between px-4 py-2 border-t">
        <div className="text-sm text-muted-foreground">
          ...
          {Math.min(currentPage * pageSize, total || 0)} of {total || 0} files
        </div>
