...
// Tailwind palette for badges (copied from IndicatorsPage for consistency)
const tailwindColors = [
  ...
  { bg: 'bg-emerald-200', border: 'border-emerald-500', text: 'text-emerald-600' },
  ...
  { bg: 'bg-fuchsia-200', border: 'border-fuchsia-500', text: 'text-fuchsia-600' },
  { bg: 'bg-neutral-200', border: 'border-neutral-500', text: 'text-neutral-600' },
...
export const CourseOfActionPage: React.FC = () => {
  ...
  const pageSize = 20;
  const { coursesOfAction, loading, error } = useCoursesOfAction({ page: currentPage, pageSize });
  const navigate = useNavigate();

  const labelColorMap = React.useRef<Map<string, (typeof tailwindColors)[0]>>(new Map());

  const results = coursesOfAction?.results || [];
  const total = coursesOfAction?.total || 0;
  const totalPages = coursesOfAction?.totalPages || 1;
    ...
    <div className="w-full flex flex-col space-y-4">
        ...
        <Table className="w-full text-sm text-foreground h-full">
          <TableHeader>
            <TableRow className="bg-muted">
              <TableHead className="font-bold p-4 text-foreground">Name</TableHead>
              <TableHead className="font-bold p-4 text-foreground">Description</TableHead>
              <TableHead className="font-bold p-4 text-foreground">Labels</TableHead>
              <TableHead className="font-bold p-4 text-foreground">Created</TableHead>
              <TableHead className="font-bold p-4 text-foreground">Modified</TableHead>
              <TableHead className="font-bold p-4 text-foreground">Revoked</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
      ...
      <div className="flex items-center justify-between px-4 py-2 border-t">
        <div className="text-sm text-muted-foreground">
          ...
          {Math.min(currentPage * pageSize, total || 0)} of {total || 0} courses of action
        </div>
