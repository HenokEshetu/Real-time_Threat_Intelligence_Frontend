...
import {
  User, Tag, Briefcase, Building2, Globe2, Mail, ChevronRight, ChevronLeft, CheckCircle2, Sparkles
} from 'lucide-react';
...
const sectorOptions = [
  'government', 'finance', 'energy', 'healthcare', 'technology', 'education', 'retail', 'other'
];

const roleOptions = [
  'admin', 'user', 'manager', 'analyst', 'operator', 'director'
];
...
export const IdentityCreatePage = () => {
  const { createIdentity, loading } = useCreateIdentity();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSuccess(false);
    try {
      const res = await createIdentity({ variables: { input: form } });
      setSuccess(true);
      setTimeout(() => navigate(`/identity/${res.data.createIdentity.id}`), 1200);
    } catch (err: any) {
          ...
          {step === 0 && (
            <div className="space-y-6 animate-fade-in">
              <h2 className="text-3xl font-extrabold bg-gradient-to-r from-blue-700 via-blue-500 to-cyan-400 bg-clip-text text-transparent drop-shadow animate-gradient-x mb-2 flex items-center gap-2">
                <Sparkles className="text-blue-400 animate-pulse" /> Create Identity
              </h2>
              <FloatingLabelInput
                label="Name"
                ...
                <div className="flex flex-wrap gap-2">
                  {identityClassOptions.map(opt => (
                    <button
                      ...
                      `}
                      onClick={() => setForm(f => ({ ...f, identity_class: opt }))}
                    >
            ...
            <div className="space-y-6 animate-fade-in">
              ...
              {success && (
                <div className="text-green-600 text-center mt-2 flex items-center justify-center gap-2">
                  <CheckCircle2 className="w-5 h-5" /> Identity created!
                </div>
              )}
            </div>
          )}
