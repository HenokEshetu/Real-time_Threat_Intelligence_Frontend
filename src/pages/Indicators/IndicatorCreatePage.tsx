...
import { CREATE_INDICATOR } from "@/graphql/indicator/mutations";
import { Indicator, CreateIndicatorInput } from "@/types/indicator";

export const IndicatorCreatePage = () => {
  const navigate = useNavigate();
  const [values, setValues] = useState<CreateIndicatorInput>({
    name: "",
    pattern: "",
    pattern_type: "",
    valid_from: "",
  ...
  return (
    <div className="min-h-screen bg-muted flex flex-col">
      <div className="w-full bg-background border-b px-4 py-3 flex items-center">
        ...
        <h1 className="text-lg font-semibold">Create Indicator</h1>
      </div>
      <form onSubmit={handleSubmit} className="max-w-xl w-full mx-auto mt-8 bg-background rounded-xl shadow p-6 space-y-6">
        ...
        <div className="pt-2">
          ...
          {error && <div className="text-destructive text-xs mt-2">Failed to create indicator.</div>}
        </div>
      </form>
    </div>
  );
