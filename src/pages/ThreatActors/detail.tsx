...
import { FaUserSecret, FaTags, FaUserEdit, FaArrowLeft, FaUserFriends, FaBolt, FaLayerGroup, FaLightbulb, FaUserAlt } from 'react-icons/fa';
  ...
  const renderBadges = (arr?: string[], color = "bg-blue-100 text-blue-800") =>
    arr?.length ? (
      <div className="flex flex-wrap gap-2">
        {arr.map((item, i) => (
          <span key={i} className={`px-2 py-1 rounded text-xs font-semibold ${color}`}>{item}</span>
        ))}
      </div>
