      ...
      <div className="max-w-3xl mx-auto px-4 py-8 relative z-10">
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center py-20 animate-fade-in">
            ...
            <div className="text-blue-700 dark:text-cyan-300 font-semibold text-lg">Loading Threat Actors...</div>
          </div>
        )}
        {/* Error display */}
        {error && (
          <div className="flex flex-col items-center justify-center py-16 animate-fade-in">
            <div className="bg-gradient-to-r from-red-100 to-red-50 border border-red-200 rounded-xl shadow-lg px-6 py-5 flex flex-col items-center">
              ...
              <div className="text-red-700 font-bold text-lg mb-1">Error loading threat actors</div>
        ...
        {selectedId && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 animate-fade-in" onClick={() => setSelectedId(null)}>
            <div
              className="bg-white/90 dark:bg-zinc-900/95 rounded-3xl shadow-2xl p-0 max-w-xl w-full relative border border-blue-100 dark:border-zinc-800 backdrop-blur-xl animate-fade-in-up overflow-hidden"
            ...
            >
              ...
              {(() => {
                const actor = actors.find((a: typeof actors[0]) => a.id === selectedId);
                ...
                return (
                  <div className="flex flex-col items-center gap-0 py-10 px-8 relative">
                    ...
                    <div className="relative mb-3">
                      ...
                      <div className="rounded-full bg-gradient-to-br from-blue-400 to-blue-700 dark:from-blue-900 dark:to-blue-700 w-20 h-20 flex items-center justify-center text-4xl font-extrabold text-white shadow-lg border-4 border-white dark:border-zinc-900 z-10 relative">
                        <span role="img" aria-label="paperclip" className="text-4xl">📎</span>
                      </div>
                    </div>
                    {/* Name and type badge */}
                    <div className="flex flex-col items-center mb-2">
                      <h2 className="text-3xl font-extrabold text-blue-900 dark:text-blue-200 tracking-tight mb-1">{actor.name}</h2>
                      <span
                        className={`inline-block px-3 py-1 rounded-full text-xs font-bold uppercase tracking-widest shadow bg-gradient-to-r ${typeColors[actor.type as string] || "from-blue-400 to-cyan-300"} text-white mb-1`}
                    ...
                    {/* Description */}
                    <div className="mb-4 text-zinc-700 dark:text-zinc-300 text-center text-base max-w-lg">{actor.description}</div>
                    {/* Labels and roles */}
                    <div className="flex flex-wrap gap-2 mb-2 justify-center">
                      {actor.labels?.map((label: string, i: number) => (
                        <span key={label} className="px-2 py-1 rounded-full text-xs font-semibold bg-gradient-to-r from-blue-300 to-cyan-200 text-blue-900 shadow">{label}</span>
                      ))}
                    </div>
                    <div className="flex flex-wrap gap-2 mb-2 justify-center">
                      {actor.roles?.map((role: string, i: number) => (
                        <span key={role} className="px-2 py-1 rounded-full text-xs font-semibold bg-gradient-to-r from-blue-200 to-blue-400 text-blue-900 shadow">{role}</span>
                      ))}
                    </div>
                    {/* Details grid */}
                    <div className="grid grid-cols-2 gap-x-6 gap-y-2 w-full text-sm text-zinc-600 dark:text-zinc-300 mt-4 mb-2">
                      <div className="flex items-center"><span className="mr-1">🧠</span><b>Sophistication:</b>&nbsp;{actor.sophistication || <span className="text-zinc-300">—</span>}</div>
                      <div className="flex items-center"><span className="mr-1">💰</span><b>Resource Level:</b>&nbsp;{actor.resource_level || <span className="text-zinc-300">—</span>}</div>
                      <div className="flex items-center"><span className="mr-1">🎯</span><b>Motivation:</b>&nbsp;{actor.primary_motivation || <span className="text-zinc-300">—</span>}</div>
                      <div className="flex items-center"><span className="mr-1">🕵️</span><b>Aliases:</b>&nbsp;{actor.aliases?.join(', ') || <span className="text-zinc-300">—</span>}</div>
                      <div className="flex items-center"><span className="mr-1">📅</span><b>First Seen:</b>&nbsp;{actor.first_seen || <span className="text-zinc-300">—</span>}</div>
                      <div className="flex items-center"><span className="mr-1">⏰</span><b>Last Seen:</b>&nbsp;{actor.last_seen || <span className="text-zinc-300">—</span>}</div>
                    </div>
                    {/* Action buttons */}
                    <div className="flex gap-4 mt-8">
                      ...
                      <button
                        ...
                        onClick={() => navigate(`/threat-actors/${actor.id}/edit`)}
                      >
