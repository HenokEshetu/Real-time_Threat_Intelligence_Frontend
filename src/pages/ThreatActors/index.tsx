...
export const ThreatActorsPage = () => {
  ...
  const { loading, error, loadMore, total, threatActors } = useThreatActors({
    filters: {},
    page: currentPage,
    pageSize: pageSize,
  });
  ...
  const handleViewThreatActor = (id: string) =>
    ...
    return (
      <div className="container py-6">
        <Alert variant="destructive">
          <AlertDescription>
            Error loading threat actors: {error.message}
          </AlertDescription>
        </Alert>
      </div>
    );
    ...
    <div className="w-full flex flex-col space-y-4">
      <div className="overflow-x-auto overflow-y-auto h-[85vh] bg-white rounded-md">
        <Table className="w-full text-sm text-foreground h-full">
          ...
          <TableBody>
            {threatActors.map((threatActor: ThreatActor) => {
              const labels = threatActor.labels || [];
              ...
              return (
                <TableRow
                  key={threatActor.id}
                  onClick={() => handleViewThreatActor(threatActor.id)}
                  className="hover:bg-gray-50 transition-colors border-b border-gray-300 cursor-pointer"
                >
                  <TableCell className="p-4 text-gray-700">
                    <Badge
                      variant="outline"
                      className="text-teal-600 border-teal-500 bg-teal-50 capitalize px-8"
                    >
                      <DramaIcon />
                      {threatActor.type.replace('-', ' ')}
                    </Badge>
                  </TableCell>
                  ...
                  <TableCell className="p-4 text-gray-600">
                    {threatActor.created
                      ? new Date(threatActor.created).toLocaleDateString()
                      : '—'}
                  </TableCell>
                  <TableCell className="p-4 text-gray-600">
                    {threatActor.modified
                      ? new Date(threatActor.modified).toLocaleDateString()
                      : '—'}
                  </TableCell>
                </TableRow>
              );
      ...
      <div className="flex items-center justify-between px-4 py-2 border-t">
        <div className="text-sm text-muted-foreground">
          ...
          {Math.min(currentPage * pageSize, total || 0)} of {total || 0} threat
          actors
        </div>
