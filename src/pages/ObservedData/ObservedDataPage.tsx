...
import { useObservedDataList } from "@/hooks/useObservedData";
import { Card, CardContent, CardTitle, CardDescription } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { useNavigate } from "react-router-dom";
import type { ObservedData } from "@/types/observeddata";
...
export const ObservedDataPage: React.FC = () => {
  const { observedData, loading, error } = useObservedDataList({});
  ...
  if (error) return (
    <div className="bg-red-50 p-4 rounded-lg text-red-600 flex items-center justify-center">
      Error loading observed data. Please try again later.
    </div>
  );

  return (
    <div className="grid grid-cols-4 gap-6 p-4 relative min-h-screen">
      {observedData.map((obs: ObservedData) => {
        const isHovered = hoveredCard === obs.id;
        ...
        return (
          <div
            key={obs.id}
            className="contents"
            onClick={() => navigate(`/observeddata/${obs.id}`)}
          >
            <Card
              ...
              onMouseEnter={() => setHoveredCard(obs.id)}
              ...
              <CardContent className="p-5 flex flex-col h-full relative z-10">
                ...
                <div className="flex flex-col gap-1">
                  <div className="text-xs text-muted-foreground">
                    <span className="font-medium">First Observed:</span> {formatDate(obs.first_observed)}
                  </div>
                  <div className="text-xs text-muted-foreground">
                    <span className="font-medium">Last Observed:</span> {formatDate(obs.last_observed)}
                  </div>
                </div>
              </CardContent>
            </Card>
